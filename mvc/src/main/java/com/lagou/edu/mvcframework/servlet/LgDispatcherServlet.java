...
public class LgDispatcherServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        //判断此控制器或此方法是否持有Security注解
        if (handler.getController().getClass().isAnnotationPresent(Security.class) ||
                handler.getMethod().isAnnotationPresent(Security.class)) {
            String[] users = null;
            //先判断并获取控制器上的注解值
            if (handler.getController().getClass().isAnnotationPresent(Security.class)) {
                users = handler.getController().getClass().getAnnotation(Security.class).value();
            }
            //若方法上同样存在此注解，则以方法上的注解值为准
            if (handler.getMethod().isAnnotationPresent(Security.class)) {
                users = handler.getMethod().getAnnotation(Security.class).value();
            }
            if (users != null && users.length > 0) {
                //获取用户名，判断此用户名是否存在于允许列表
                String username = req.getParameter("username");
                boolean check = Arrays.asList(users).contains(username);
                if (!check) {
